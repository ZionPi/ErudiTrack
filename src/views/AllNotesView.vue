<template>
  <div class="all-notes-view">
    <div class="top-bar">
      ...
      <h1>我的笔记</h1>
      <el-button style="position: absolute; top: 0; right: 0;" type="primary" @click="openChartDialog">
        统计图表
      </el-button>
    </div>
  ...
  methods: {
    ...
    disposeChart() {
      ...
      filteredNotesForChart.forEach(note => {
        const date = this.formatDateForChart(note.timestamp);
      ...
      if (this.chartType === 'bar') {
        ...
        const scatterData = filteredNotesForChart.map(note => [new Date(note.timestamp)]);
        option = {
          title: { text: '笔记时间分布' },
    ...
    //     option = {
    //       title: { text: '笔记时间分布' },
    //       tooltip: { formatter: (params) => this.formatDate(params.value[0]) }, // Access the Date object at index 0
    //       xAxis: { type: 'time' },
    //       yAxis: { type: 'category', data: ['笔记'] },
    //       series: [{ type: 'scatter', symbolSize: 10, data: scatterData.map(item => [item[0], '笔记']) }]
    //     };
